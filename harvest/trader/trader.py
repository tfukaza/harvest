...
class Trader:
    ...
    def _setup(self, interval, aggregations, sync=True):
        ...
        logging.debug(f"Watchlist: {self.watch}")
        ...
        logging.debug(f"Aggregations: {self.aggregations}")
    ...
    def _setup_stats(self):
        ...
        logging.debug("Fetching positions")
        # Get positions
        ...
        logging.debug("Fetching option data")
        # Update option stats
    ...
    def timeout(self):
        logging.debug("Begin timer")
        time.sleep(1)
        if not self.all_recv:
            logging.debug("Force flush")
            self.flush()
    ...
    def main(self, df_dict):
        ...
        logging.debug(f"Got data for: {symbols}")
        self.needed = list(set(self.needed) - set(symbols))
        logging.debug(f"Still need data for: {self.needed}")
 
        self.block_queue.update(df_dict)
        logging.debug(self.block_queue)
    ...
    def _update_order_queue(self):
        ...
        """
        logging.debug(f"Updating order queue: {self.order_queue}")
        for i, order in enumerate(self.order_queue):
            ...
            logging.debug(f"Updating status of order {order['id']}")
            self.order_queue[i] = stat

        logging.debug(f"Updated order queue: {self.order_queue}")
        new_order = []
        order_filled = False
    ...
    def _update_stats(self, df_dict, new=False, option_update=False):
        ...
        logging.debug(f"Stock positions: {self.stock_positions}")
        logging.debug(f"Option positions: {self.option_positions}")
        logging.debug(f"Crypto positions: {self.crypto_positions}")
    ...
    def buy(self, symbol: str, quantity: int, in_force: str, extended: bool):
        ...
        logging.debug(f"BUY: {self.timestamp}, {symbol}, {quantity}")
        logging.debug(f"BUY order queue: {self.order_queue}")
    ...
    def sell(self, symbol: str, quantity: int, in_force: str, extended: bool):
        ...
        if ret == None:
            ...
            return None
        self.order_queue.append(ret)
        logging.debug(f"SELL: {self.timestamp}, {symbol}, {quantity}")
        logging.debug(f"SELL order queue: {self.order_queue}")
        asset_type = 'crypto' if is_crypto(symbol) else 'stock'

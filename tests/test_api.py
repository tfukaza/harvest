...
class TestAPI(unittest.TestCase):
    ...
    def test_timeout(self):
        ...
        self.assertEqual(a_cur['A']['close'][-1],       t.storage.load('A', Interval.MIN_1)['A']['close'][-2])
        self.assertEqual(data['A']['A']['close'][-1],   t.storage.load('A', Interval.MIN_1)['A']['close'][-1])
        # Check if B has been duplicated
        self.assertEqual(b_cur['B']['close'][-1],       t.storage.load('B', Interval.MIN_1)['B']['close'][-2])
        self.assertEqual(b_cur['B']['close'][-1],       t.storage.load('B', Interval.MIN_1)['B']['close'][-1])
    ...
    def test_timeout_cancel(self):
        ...
        # Check if A has been added to storage
        self.assertEqual(a_cur['A']['close'][-1],       t.storage.load('A', Interval.MIN_1)['A']['close'][-2])
        self.assertEqual(data_a['A']['A']['close'][-1], t.storage.load('A', Interval.MIN_1)['A']['close'][-1])
        # Check if B has been added to storage
        self.assertEqual(b_cur['B']['close'][-1],       t.storage.load('B', Interval.MIN_1)['B']['close'][-2])
        self.assertEqual(data_b['B']['B']['close'][-1], t.storage.load('B', Interval.MIN_1)['B']['close'][-1])
